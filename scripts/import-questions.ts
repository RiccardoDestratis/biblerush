...
async function importQuestionSet(questionSet: QuestionSet, orderIndex: number = 0): Promise<void> {
  console.log(`\n📦 Importing question set: ${questionSet.set_name.en}`);
  ...
  if (setError) {
    console.error(`❌ Error inserting question set:`, setError);
    throw setError;
  }

  console.log(`✅ Question set created with ID: ${insertedSet.id}`);
  ...
  for (let i = 0; i < questionSet.questions.length; i++) {
    ...
    console.log(`   ✅ Question ${i + 1}/${questionSet.questions.length} imported`);
  }

  console.log(`✅ Successfully imported question set: ${questionSet.set_name.en}`);
}
...
async function main() {
  ...
  console.log(`\n📊 Found ${data.question_sets.length} question set(s) to import`);
  ...
  for (let i = 0; i < data.question_sets.length; i++) {
    try {
      ...
      console.error(`\n❌ Failed to import question set ${i + 1}:`, error);
      process.exit(1);
    }
  }

  console.log(`\n🎉 Successfully imported all question sets!`);
}
